...
struct search {
    ...
    bool guarantee_win(State state) {
        ...
        for (uint64_t size = 1; size <= BOARD_SIZE; size++) {
            ...
            if (count > 0) {
                for (uint64_t partition = 0; partition < ceil(size / 2.0); partition++) {
                    
                    // 1-wide partitions
                    auto new_state = state;
                    new_state.turn = !state.turn;
                    new_state.set(size, count - 1);
                    if (size > 1) new_state.set(size - 1 - partition, new_state.select(size - 1 - partition) + 1);
                ...
                for (uint64_t partition = 0; partition < ceil((size - 1) / 2.0); partition++) {
                    // 2-wide partitions
                    auto new_state = state;
                    new_state.turn = !state.turn;
                    new_state.set(size, count - 1);
                    if (size > 2) new_state.set(size - 2 - partition, new_state.select(size - 2 - partition) + 1);
                    if (partition > 0) new_state.set(partition, new_state.select(partition) + 1);
